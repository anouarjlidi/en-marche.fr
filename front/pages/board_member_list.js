...
export default (api) => {
    ...
    function addMemberToList(event) {
        ...
        api.addBoardMemberToList(element.dataset.memberid).then(() => {
            ...
            element.className = element.className.replace('btn-add-member-list', 'btn-remove-member-list newbtn--green');
        }).catch((err) => {
            element.innerHTML = 'Sauvegarder ce profil';
            window.alert('Nous n\'arrivons pas à ajouter cette personne à votre liste. Actualisez votre page et réessayez !');
        });
    }
    ...
    function removeMemberToList(event) {
        ...
        api.deleteBoardMemberOnList(element.dataset.memberid).then((response) => {
            element.className = element.className.replace('btn-remove-member-list newbtn--green', 'btn-add-member-list');
            reloadIfNeeded(element);
            element.innerHTML = 'Sauvegarder ce profil';
        }).catch((err) => {
            element.innerHTML = 'Profil sauvegardé';
            window.alert('Nous n\'arrivons pas à supprimer cette personne de votre liste. Actualisez votre page et réessayez !');
        });
    }
